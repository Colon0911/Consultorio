...
public class ControladorExpedientes {
     ...
     * @param exp
     */
    public void cargarMedico(FrmSesion frmsesion,  ExpedienteSencillo exp) {
        ...
        this.medico.setId(frmsesion.getTxtUsuario().getText());
    ...
    public void acutalizarExpediente(ExpedienteSensillo exp) {
        BasedeDatos bd = new BasedeDatos("UPDATE `expediente` SET `Descripcion`=?  WHERE Medico=? and Fecha=? AND Hora=? AND Paciente=?");
        SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");
        java.util.Date utilDate = formato.parse(exp.getTxtFechaAtencion().getText());
        java.sql.Date fecha = new java.sql.Date(utilDate.getTime());
        this.medico = new Medicos();
